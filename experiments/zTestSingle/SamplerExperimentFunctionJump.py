...
def run_diagonal_walk_experimentJump(num_samples, D_values=None, seed=None, JUMP=0.0):
    """
    ...
    ----------
    num_samples : int
        Number of samples to draw for each D.
    D_values : iterable[int] or None
        The D values to test. Defaults to 0..10.
    seed : int or None
        Random seed.
    JUMP : float
        Probability of ignoring the local D constraint at a given step and
        instead drawing a completely global/random sample.

    Returns
    -------
    dict
        Dictionary keyed by D. For each D, the value is a dict with:
          - "trace_counts":
    ...
    """
    if not (0.0 <= JUMP <= 1.0):
        raise ValueError("JUMP must be between 0 and 1.")
    ...
    def sample_one(
        ...
        # With probability JUMP, ignore the local window and sample globally.
        do_jump = (current_state is not None) and (rng.random() < JUMP)

        if current_state is not None and not do_jump:
        ...
        return decoded, elapsed, do_jump
    ...
    for D in D_values:
        ...
        results[D] = {
            ...
            "jumps": jumps,  # True if that step ignored the local constraint
        }
