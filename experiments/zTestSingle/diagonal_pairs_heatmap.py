...
def build_problem_clauses():
    ...
    # Only allow diagonal pairs: x0 == x1.
    for a in DOMAIN:
        for b in DOMAIN:
            if a != b:
                clauses.append([
...
def sample_one(clauses, sampling_set, decode, rng):
...
def update_plot(fig, bars0, bars1, im, x0_counts, x1_counts, pair_counts, n):
    ...
    fig.suptitle(f"Diagonal-pair sampling — samples seen: {n}", fontsize=13)
    fig.canvas.draw()
...
def run_demo(num_samples, refresh_every, delay, seed, change_after):
    ...
    clauses, sampling_set, decode = build_problem_clauses()
    ...
    for i in range(1, num_samples + 1):
        if change_after is not None and i == change_after + 1:
            # Demo dynamic change: after this point only allow diagonal values >= 5.
            for v in range(5):
                clauses.append([-next(k for k, val in decode.items() if val == ("x0", v))])
                clauses.append([-next(k for k, val in decode.items() if val == ("x1", v))])

        s = sample_one(clauses, sampling_set, decode, rng)
        samples.append(s)
        x0_counts[s["x0"]] += 1
        x1_counts[s["x1"]] += 1
        ...
        if i == 1 or i % refresh_every == 0 or i == num_samples:
            update_plot(fig, bars0, bars1, im, x0_counts, x1_counts, pair_counts, i)
            if delay > 0:
                plt.pause(delay)
